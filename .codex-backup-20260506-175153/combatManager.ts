...
export function canControlActor(session: Session, userId: string, actor: CombatActor): boolean {
  ...
  const entity = getActorEntity(session, actor)
  return !!entity && "userId" in entity && entity.userId === userId
}
...
export function addCombatLog(session: Session, message: string): CombatLogEntry {
  const entry = { id: nextId("log"), message, createdAt: Date.now() }
...
export function startPendingAction(
  ...
  const pending = { ...action, createdAt: Date.now() }
...
export function consumeRollemRoll(
  ...
  if (pending.type === "attack") {
    ...
    const message = `${getActorLabel(session, pending.source)} attacks ${getActorLabel(session, pending.target)} with ${numericValue}.`
    addCombatLog(session, `${message} Waiting for Dodge or Take Hit.`)
